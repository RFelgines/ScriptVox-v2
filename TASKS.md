...
- ✅ **Fixtures EPUB stabilisées** (2026-07-07) — `tests/check_phase2.py` écrivait directement dans
  `tests/fixtures/test.epub` et `test_whitespace.epub` à chaque run ; ebooklib tamponne un nouveau
  `dcterms:modified` à chaque `write_epub()`, d'où un diff binaire permanent sous git malgré un
  contenu identique. Le round-trip écriture+parsing passe désormais par un fichier temporaire
  (`tempfile.NamedTemporaryFile`, nettoyé via `os.unlink`) ; la fixture trackée n'est (ré)écrite que
  si elle n'existe pas encore (bootstrap sur clone frais, no-op sinon). Vérifié idempotent sur 2 runs
  consécutifs (`git status` propre après chaque run). Fichier (1) : `tests/check_phase2.py`.
- ✅ **`DELETE /voices/{id}` nullifie `Character.voice_id` orphelin** (2026-07-07) — avant de
  ...
  Fichiers (2) : `app/api/routes/voices.py`, `tests/check_phase17.py`.
- ✅ **README complété pour un public externe** (2026-07-07) — encart d'avertissement en tête
  (local/sans auth/ne pas exposer, confidentialité EdgeTTS→Microsoft / Gemini→Google, limites
  connues CSRF localhost + EPUB non borné en décompression) ; table de config complétée
  (`DATA_DIR`, `FRONTEND_ORIGINS`, 4 variables de timeout Ollama) ; section Tests réécrite avec les
  32 suites existantes (contre 15 avant) sous forme de tableau + note sur les numéros manquants
  (13/18/20/22, repris dans une autre suite ou frontend-only) + boucle PowerShell/bash pour tout
  lancer d'un coup (triée numériquement, vérifiée) ; version Python (3.11+, garde `audioop`
  `audioop-lts` pour ≥3.13) ajoutée en tête de Setup. Fichier (1) : `README.md`.
...
## Phase 25 — Orbe de voix « vivante » (itération 2 du chantier orbes) (2026-07-18)

> **Contexte.** Retour utilisateur sur le design « Glass Bubble » mis en prod en Phase 22 : résultat
> « perfectible », souhait d'une orbe qui « vit » pendant la lecture, référence explicite aux orbes
> ChatGPT / ElevenLabs, liberté explicitement accordée de s'éloigner de ce qui avait été défini.
> Exigence maintenue : différence nette entre l'état statique (ne bouge pas) et l'état actif (parle).

### Livré (2026-07-18) — `VoiceOrb.tsx` réécrit en « orbe vivante »

**Design.** Sphère lumineuse dans la teinte de la voix (identité golden-angle inchangée) composée de :
- 2-3 **nappes de couleur** (radial-gradients à bords doux, teintes analogues ±28° + contre-couleur
  discrète, blend `screen`) en dérive organique sur des durées **incommensurables** (5.9 / 7.3 / 9.1 s)
  → le motif combiné ne boucle jamais visiblement ;
- un **cœur blanc scintillant à deux fréquences superposées** (1.13 s et 1.87 s) dont la somme paraît
  quasi aléatoire — simule la modulation d'une voix sans analyse audio réelle ;
- un **halo** qui pulse au-delà du cercle (1.6 s) uniquement en lecture ;
- une **respiration** de la sphère (2.1 s) + finition sphérique (reflet zénithal, ombres internes
  proportionnelles à la taille 16→160 px).

**États.** Actif = tout s'anime + saturation/luminosité rehaussées + halo. Statique = toutes les
animations en `animation-play-state: paused` (pas `animation: none` : la reprise repart de la pose
figée, sans saut) + désaturation/assombrissement (`filter` transitionné 500 ms) + cœur presque éteint.
Chaque voix a une **pose statique déterministe qui lui est propre** (délais négatifs dérivés de la
teinte) — deux orbes actives ne bougent jamais en miroir.

**Performance (contrainte 50-200 orbes de la transcription maintenue).** Compositor-only : transform +
opacity uniquement, **plus aucun `filter: blur` ni `backdrop-filter` par instance** (le Glass Bubble
en avait un de chaque, coûteux en repaint) ; animations en pause = zéro coût ; `willChange` posé
seulement quand `active` ; en dessous de 28 px une nappe est retirée ; le montage paresseux
IntersectionObserver de `ChapterTranscript` est inchangé. `prefers-reduced-motion` respecté (pause
globale via `.living-orb`).

**API inchangée** (`hue`, `size`, `className`, `children`, `active`) — zéro modification des 4 sites
d'appel (voix/page, PlayerBar, ChapterTranscript, books/[id]).

**Fichiers (4).**
- `frontend/src/components/VoiceOrb.tsx` — réécriture complète.
- `frontend/src/app/globals.css` — nouvelles keyframes `orb-*` ; suppression des anciennes
  (`orbSpinSlow`/`orbGlassBreathe`) et du bloc mort `.voice-orb`/`voice-orb-shift` (plus référencé
  nulle part depuis la Phase 22).
- `frontend/src/app/orb-lab/page.tsx` — carte « Orbe vivante (prod) » ajoutée en tête du labo
  (le labo reste conservé à dessein, cf. Phase 22).
- `TASKS.md` — cette entrée.

**Piste future (non faite, à trancher si besoin).** Réactivité audio réelle : brancher un
